...
describe('爬虫站点https://www.agemys.com的测试', () =>{
    it('获取详情番剧', () => {
        ...
        .then(detail => {
            ...
            expect(() => diff(detail, diffData)).toBe(true)
        })

        // 一个错误的请求示例
        ACGTV.getDetailById('不存在的id')
        .catch(err => {
            expect(() => {
                return err instanceof BaseExceotion
            }).toBe(true)
        })
    })
})
